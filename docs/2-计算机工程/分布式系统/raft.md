本质上是为了实现**主备复制**。
Mysql也能主备复制，为啥不需要共识算法呢，因为主备的关系是确定的。而在分布式下的主备复制，是因为随时都可能发生主机宕机，这时候备机就可以成为主机，这个时候是不确定的，需要所有存活的备机对谁来当主机达到一个共识，这就需要共识算法了。
主机选好了，接下来就是常规的主备复制模式了。


所以raft有两个part：1.确认主机2.主备复制
